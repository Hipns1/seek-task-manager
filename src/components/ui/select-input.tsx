...
export const SelectInput = <Label extends string, Value extends string>(
  props: SelectInputProps<Label, Value>
) => {
  const {
    className,
    labelKey = "label",
  ...
  const memoizedOptions = useMemo(() => {
    ...
    return items.filter((element) =>
      normalizeString(element[labelKey as Label].toLowerCase()).includes(
  ...
  return (
    <div className={cn("flex flex-col gap-1", className)}>
                  ...
                  <div className="flex max-w-[90%] items-center truncate">
                    ...
                    {valueItem ? (
                      memoizedOptions.find(
                        (item) => item[valueKey as Value] === valueItem
                      )?.[labelKey as Label]
                    ) : (
      ...
      {error && (
        <span className="text-xs text-[#FF432C]">*{error?.message ?? ""}</span>
      )}
    </div>
  );
};
